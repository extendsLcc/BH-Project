...
public class FixedColumnTable implements ChangeListener, PropertyChangeListener
{
	...
	public FixedColumnTable(int fixedColumns, JScrollPane scrollPane)
	{
		...
		fixed = new JTable();
		fixed.setSelectionBackground( new Color( 204, 204, 204 ) );
		fixed.setModel( model = new DefaultTableModel( new Object[]{ "" }, 0 ){
		...
		//fixed.getColumnModel().getColumn( 0 ).setMinWidth( 80 );
		//fixed.getColumnModel().getColumn( 0 ).setPreferredWidth( 80 );
		DefaultTableCellRenderer  centerRenderer = new DefaultTableCellRenderer();
		centerRenderer.setHorizontalAlignment( SwingConstants.CENTER );
		fixed.setDefaultRenderer( Object.class , centerRenderer );
		centerRenderer.setBackground( scrollPane.getBackground() );
		
		int min = 0, hour = 8;
		
		while( !( hour >= 19 && min == 5 ) ){
			
			model.addRow( new Object[]{ ( ( hour + "" ).length() == 1 ? "0" + hour : hour ) + ":" + ( ( min + "" ).length() == 1 ? "0" + min : min ) } );
			min += 5;
			if( min == 60 ){
	...
	public int getTimeRowIndex( String time ){
		
		for( int i = 0; i < model.getRowCount(); i++ ){
			
			if( time.equalsIgnoreCase( (String) model.getValueAt( i, 0 ) ) ){
	...
	public String getTimeByRow( int row ){
